...
const storeLabels: Record<string, string> = {
    ...
    image_files: "图片文件",
    media_files: "媒体文件",
    image_generation_logs: "图片生成记录",
    video_generation_logs: "视频生成记录",
    agent_chat_messages: "Agent 对话",
    prompt_cache: "提示词缓存",
    prompt_sources: "提示词来源",
    projects: "画布项目",
    assets: "资产",
};
            ...
            <section className="rounded-lg border border-stone-200 p-4 dark:border-stone-800">
                <div className="flex flex-wrap items-start justify-between gap-3">
                    <div>
                        ...
                        <div className="mt-1 text-xs text-stone-500">查看浏览器为本站点分配的空间与各数据分类的占用情况。</div>
                    </div>
                ...
                {!usage && loading ? (
                    ...
                    <>
                        <div className="mt-4 grid gap-3 sm:grid-cols-3">
                            <StorageMetric icon={<Database className="size-4" />} label="内容数据占用" value={formatStorageBytes(indexedDbBytes)} hint="IndexedDB 内容估算" />
                            <StorageMetric icon={<HardDrive className="size-4" />} label="站点已用空间" value={formatStorageBytes(usage.usage)} hint="浏览器统计的站点总占用" />
                            <StorageMetric icon={<Layers3 className="size-4" />} label="站点配额" value={formatStorageBytes(usage.quota)} hint="浏览器为该站点分配的上限" />
                        </div>
                        <div className="mt-4">
