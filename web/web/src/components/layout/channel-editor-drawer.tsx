        ...
        >
            <div className="grid gap-4 md:grid-cols-2">
                ...
                <label className="block md:col-span-2">
                    ...
                    <span className="mt-1 block text-xs text-stone-500">直连中转站时填写根地址（例如 https://pdhlzy.com）；系统会自动补 /v1。保存后点击「选择模型」拉取并勾选模型。</span>
                    <span className="mt-1 block text-xs text-stone-500">“Gemini 原生接口”仅用于 Google 的 /v1beta 接口；中转站即使模型名以 Gemini 开头，通常仍应选 OpenAI。</span>
                </label>
                <label className="block md:col-span-2">
                    <span className="mb-1 block text-sm font-medium">API Key</span>
                    <Input.Password value={draft.apiKey} onChange={(event) => patch({ apiKey: event.target.value })} placeholder="sk-..." />
                    {draft.id === "banana-fast" ? <span className="mt-1 block text-xs text-stone-500">此处留空时，会沿用同一接口地址下另一个已配置渠道的 Key；不会复制或导出该 Key。</span> : null}
                </label>
            </div>
