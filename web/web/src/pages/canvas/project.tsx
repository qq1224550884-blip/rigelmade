...
function InfiniteCanvasPage() {
    ...
    const removeNodeInputReference = useCallback((nodeId: string, referenceNodeId: string) => {
        const configConnection = connectionsRef.current.find((connection) => connection.fromNodeId === nodeId && nodesRef.current.find((item) => item.id === connection.toNodeId)?.type === CanvasNodeType.Config);
        const targetNodeId = configConnection?.toNodeId || nodeId;
        setConnections((prev) => prev.filter((connection) => !(connection.fromNodeId === referenceNodeId && connection.toNodeId === targetNodeId)));
    }, []);
    ...
    const handleNodeContextMenu = useCallback((event: ReactMouseEvent, nodeId: string) => {
        if (event.target instanceof Element && event.target.closest('input, textarea, [contenteditable="true"], [role="textbox"]')) return;
        event.preventDefault();
        event.stopPropagation();
    ...
    const renderNodePanel = useCallback(
        (panelNode: CanvasNodeData) =>
            getNodeDefinition(panelNode.type)?.Panel ? (
                ...
                <CanvasNodePromptPanel
                    ...
                    onRemoveReference={removeNodeInputReference}
        ...
        [configInputsById, confirmStopGeneration, handleConfigNodeChange, handleGenerateNode, handleNodePromptChange, mentionReferencesByNodeId, removeNodeInputReference, renderPluginPanel, runningNodeId],
    );
