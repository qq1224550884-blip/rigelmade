...
export default function ImagePage() {
    ...
    const [pricing, setPricing] = useState<{ model: string; quality: string; credits: number }[]>([]);
    const [balance, setBalance] = useState(0);
    const modelBaseName = modelOptionName(model);
    const unitCost = pricing.find((item) => item.model === modelBaseName && item.quality === effectiveConfig.quality)?.credits ?? 0;
    const totalCost = unitCost * generationCount;

    useEffect(() => {
        let cancelled = false;
        Promise.all([
            commercialRequest<{ prices: { model: string; quality: string; credits: number }[] }>("/api/catalog/pricing"),
            commercialRequest<{ balance: number }>("/api/billing/ledger"),
        ]).then(([priceRes, ledgerRes]) => {
            if (cancelled) return;
            setPricing(priceRes.prices);
            setBalance(ledgerRes.balance);
        }).catch(() => {});
                    ...
                    <div className="thin-scrollbar flex flex-col rounded-lg border border-stone-200 bg-card p-4 shadow-sm dark:border-stone-800 lg:min-h-0 lg:overflow-y-auto">
                        ...
                        <div className="mt-auto space-y-2 pt-6">
                            <div className="flex items-center justify-between text-xs text-stone-500 dark:text-stone-400">
                                <span>{unitCost ? `消耗 ${unitCost} 积分/张 × ${generationCount} = ${totalCost} 积分` : "积分消耗加载中"}</span>
