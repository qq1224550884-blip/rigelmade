...
function parseGeminiImagePayload(payload: GeminiPayload) {
    validateGeminiPayload(payload);
    const dataUrls = collectGeminiImageValues(payload);
    if (dataUrls.length) return dataUrls.map((dataUrl) => ({ id: nanoid(), dataUrl }));
    ...
    const text = (payload.candidates || [])
        ...
        .trim()
        .slice(0, 160);
    const finishReason = (payload.candidates || []).map((candidate) => candidate.finishReason).filter(Boolean).join(", ");
    const detail = [finishReason ? `结束原因：${finishReason}` : "", text ? `上游文本：${text}` : ""].filter(Boolean).join("；");
    throw new Error(`Gemini 接口未返回可读取的图片${detail ? `（${detail}）` : ""}。请确认该模型具有生图能力；若中转站实际使用 OpenAI chat/completions，请把渠道协议改为 OpenAI 并使用调用脚本。`);
}

/** Accept native Gemini images plus the common OpenAI-compatible shapes returned by Gemini proxy services. */
function collectGeminiImageValues(payload: GeminiPayload): string[] {
    const values: string[] = [];
    const seen = new Set<string>();
    const add = (value: unknown) => {
        if (typeof value !== "string" || !value.trim()) return;
        const normalized = value.trim();
        if (!/^data:image\//i.test(normalized) && !/^https?:\/\//i.test(normalized)) return;
    ...
    const addRecord = (item: unknown) => {
        if (typeof item === "string") {
            add(item);
            for (const match of item.matchAll(/data:image\/[-\w.+]+;base64,[A-Za-z0-9+/=\r\n]+|https?:\/\/[^\s)\]"'<>]+/g)) add(match[0].replace(/[.,]+$/, ""));
            return;
        }
        if (!item || typeof item !== "object") return;
        const record = item as Record<string, unknown>;
        if (typeof record.b64_json === "string") add(`data:image/png;base64,${record.b64_json}`);
        add(record.url);
        add(record.image_url);
        if (record.image_url && typeof record.image_url === "object") add((record.image_url as Record<string, unknown>).url);
        add(record.imageUrl);
        add(record.file_uri);
        add(record.fileUri);
    };
    ...
    for (const part of payload.candidates?.flatMap((candidate) => candidate.content?.parts || []) || []) {
        ...
        if (inlineData?.data) add(`data:${inlineData.mimeType || "image/png"};base64,${inlineData.data}`);
        add(part.fileData?.fileUri);
        addRecord(part);
    }

    const proxyPayload = payload as unknown as Record<string, unknown>;
    for (const key of ["data", "images", "results"]) {
        const items = proxyPayload[key];
        if (Array.isArray(items)) items.forEach(addRecord);
    }
    const choices = proxyPayload.choices;
    if (Array.isArray(choices)) {
        choices.forEach((choice) => {
            ...
            if (Array.isArray(content)) content.forEach(addRecord);
            else addRecord(content);
            addRecord(message);
        });
    }
    return values;
